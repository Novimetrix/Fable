# EM-LOCK-DIAG.ps1 — inject a tiny diagnostic panel to report computed sizes
...
# One-line inline script to avoid quoting issues
$js = @'
(function(){
  try{
    var doc = document, de = doc.documentElement, b = doc.body;
    function px(x){ return parseFloat(x||''0''); }
    ...
    doc.getElementById(''em-lock-diag-close'').onclick=function(){ box.remove(); };
  }catch(e){console&&console.warn&&console.warn(''EM-LOCK DIAG error'',e);}
})();
'@
$scriptTag = '<script id="em-lock-diag">'+$js+'</script>'
...
foreach($f in $files){
  ...
  try{
    ...
    $idx = $txt.IndexOf('<script id="em-lock-diag">', [StringComparison]::OrdinalIgnoreCase)
    if ($idx -ge 0){
