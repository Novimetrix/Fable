...
# === NM PATCH: Normalize lazy-load images (data-src / data-srcset -> src / srcset) ===
# Proper here-strings: opener and closer on their own lines.
Get-ChildItem -Recurse -File -Include *.html | ForEach-Object {
    $html = Get-Content $_.FullName -Raw

    $pat1 = @'
<img([^>]*?)\s(?:data-lazy-src|data-src)=["']([^"']+)["']([^>]*)>
'@
    $rep1 = @'
<img$1 src="$2"$3>
'@
...
\sloading=["']lazy["']
'@
    $html = $html -replace $patLazy, ''

    $patDec = @'
\sdecoding=["']async["']
'@
    $html = $html -replace $patDec, ''

    $pat3 = @'
<img((?:(?!\ssrc=).)*?)\ssrcset=["']([^"']+)["']([^>]*)>
'@
    $html = $html -replace $pat3, {
...
Write-Host "✅ Lazy-load images normalized (data-src → src)."
# === /NM PATCH END ===
